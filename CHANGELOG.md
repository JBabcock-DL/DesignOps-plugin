...
### Added — MCP payload transport (`21`) + draw order in `13`

- **[`skills/create-component/conventions/21-mcp-ephemeral-payload-protocol.md`](skills/create-component/conventions/21-mcp-ephemeral-payload-protocol.md)** — Normative two-axis model (**transport** vs **wire size**), per-slice `assemble-slice` checklist, **`mcp-<slug>.json`** naming, pairing with **`18`**. Linked from **`AGENTS.md`**, [`00-overview.md`](skills/create-component/conventions/00-overview.md), [`memory.md`](memory.md). [`EXECUTOR`](skills/create-component/EXECUTOR.md), [`08-cursor-composer-mcp`](skills/create-component/conventions/08-cursor-composer-mcp.md).
- **Draw order** documented in [`13-component-draw-orchestrator.md`](skills/create-component/conventions/13-component-draw-orchestrator.md) (`SLUG_ORDER`). *(Retired in-tree: separate `22-deterministic-agent-flows.md` — procedural content folded into `13` / `AGENTS` / `21`.)*

- **`scripts/designops-step6-engine.mjs`** (`npm run designops:step6:*`), **`scripts/lib/designops-step6-status.mjs`**, **`scripts/config-projection.mjs`** + **`scripts/config-projection-map.json`** (Tier 1 hook in **`assemble-slice`**), **`scripts/qa-assembled-size.mjs`**, **`scripts/qa-config-projection.mjs`** (`npm run qa:config-projection`; wired into **`npm run verify`**), **`scripts/designops-canvas-session.mjs`**. **[`skills/create-component/conventions/23-designops-step6-engine.md`](skills/create-component/conventions/23-designops-step6-engine.md)**. *(Follow-on engine-split notes were in a removed doc; use **`18`** + `build-min` when pursuing smaller doc engines.)*
### Changed — MCP payloads: ephemeral staging allowed

- **`AGENTS.md`**, **[`.cursor/rules/mcp-inline-payloads.mdc`](.cursor/rules/mcp-inline-payloads.mdc)**, [`CLAUDE.md`](CLAUDE.md), [`memory.md`](memory.md), [`18-mcp-payload-budget.md`](skills/create-component/conventions/18-mcp-payload-budget.md) — Default remains **inline** `use_figma` `code`; **`assemble-slice --out`** / **`--emit-mcp-args`** + parent **`Read`** → `call_mcp` is documented when IDE transport needs it — still **no** persistent staging under **`skills/`** or duplicate copies of committed bundles.

### Added — MCP payload shrink roadmap *(superseded — see `18` + `build-min`)*

- *(Historical entry; the standalone roadmap doc was later removed. Current north star: [`18-mcp-payload-budget.md`](skills/create-component/conventions/18-mcp-payload-budget.md).)*

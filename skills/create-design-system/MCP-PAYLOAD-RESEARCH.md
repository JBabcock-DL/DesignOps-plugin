...
**Handoff / transport checklist:** [`upstream/AGENT-HANDOFF-ISSUES.md`](./upstream/AGENT-HANDOFF-ISSUES.md) · [`upstream/README.md`](./upstream/README.md). **Withdrawn** file-backed transport stub (historical only): [`RFC-figma-mcp-bundle-transport.md`](./RFC-figma-mcp-bundle-transport.md).

**Stable fix (this repo):**

| Layer | What | Where |
|--------|------|--------|
| **Plugin package** | Committed **one-file** MCP payloads for Step **15a** (and 15b/15c/17) | [`canvas-templates/bundles/step-15a-primitives.mcp.js`](./canvas-templates/bundles/step-15a-primitives.mcp.js) (= `_lib.js` + `primitives.js` + [`bundles/_step15a-runner.fragment.js`](./canvas-templates/bundles/_step15a-runner.fragment.js)). Regenerate via [`scripts/bundle-canvas-mcp.mjs`](./scripts/bundle-canvas-mcp.mjs). |
...
| **Wire transport** | Editor **`Read`** `.min.mcp.js` → inline `use_figma` → `code` only (~50k cap) | [`conventions/17-table-redraw-runbook.md`](./conventions/17-table-redraw-runbook.md), [`AGENTS.md`](../../AGENTS.md) |

**Out of scope:** No hosted-URL / CDN tier for bundles — distribution is **plugin-shipped** artifacts only.
...
| **Agent cannot “lift” 36k chars** | Cursor `Read` of source adds **line-number prefixes**; reconstructing one giant string for `call_mcp_tool` from chat is brittle; base64 chunk reassembly across tool outputs is error-prone. | Use a host path that supplies the **full** bundle bytes to the MCP tool argument (editor `Read` of `.min.mcp.js` is the documented happy path). There is **no** shipping `codeWorkspacePath` — see withdrawn [RFC stub](./RFC-figma-mcp-bundle-transport.md). |
| **Shell `cat` truncation** | Terminal tool output capped (~20k chars) → incomplete `code` pasted into MCP. | **Never** use full-bundle `cat` as source of truth; editor **`Read`** the `.min.mcp.js` then pass inline `code` ([`AGENTS.md`](../../AGENTS.md) § *Host vs agent transport*). |
...
| 2026-04-21 | **Upstream pack:** filing checklist + MRE/acceptance/validation docs under `upstream/` (later **removed** — file-backed `code` not shipped in MCP schema). |
| 2026-04-21 | **Doc correction:** RFC replaced with **withdrawn** stub; repo happy path = **`Read` → inline `code` only**; removed Option D upstream markdown files; updated AGENTS, rules, conventions 16–17, skills, handoff doc. |
| 2026-04-20 | **Step 17 bundle:** [`canvas-templates/token-overview.js`](./canvas-templates/token-overview.js) + [`bundles/_step17-runner.fragment.js`](./canvas-templates/bundles/_step17-runner.fragment.js); `step-17-token-overview.min.mcp.js` **~19.8k** chars (well under 50k cap). Runner returns `{ ok: false, skipped: 'page missing' }` when **↳ Token Overview** is absent. Runbook [`conventions/17-table-redraw-runbook.md`](./conventions/17-table-redraw-runbook.md); sync **6.Canvas.9d** uses the same bundle as create Step 17. |

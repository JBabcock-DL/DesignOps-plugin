...
**MCP canvas runs (happy path — Steps 15a, 15b, 15c):** **Host-first, `Read` second, never full-bundle `cat`:** If the MCP host supports an allow-listed **file-backed** `code` path for `use_figma`, prefer it (see [`RFC-figma-mcp-bundle-transport.md`](./RFC-figma-mcp-bundle-transport.md) Option D — [tracking issue](https://github.com/JBabcock-DL/DesignOps-plugin/issues/4)). Otherwise, `Read` the matching committed **`.min.mcp.js`** bundle and pass its contents **verbatim** as `use_figma` → `code` (plus `fileKey`, `description`, `skillNames: "figma-use"`). Do **not** dump the bundle via shell `cat` / `type` and copy from truncated terminal output. Bundles resolve rows + IDs in-plugin; **you never assemble `ctx` in the tool call**, and **never write a staging file** to JSON-escape the payload. Transport detail: [`conventions/16-mcp-use-figma-workflow.md`](./conventions/16-mcp-use-figma-workflow.md) § *Host vs agent transport*. Bundle map:

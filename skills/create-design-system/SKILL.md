...
**MCP canvas runs (happy path — Steps 15a–15c + Step 17 Token Overview):** **`Read` the bundle, never full-bundle `cat`:** `Read` the matching committed **`.min.mcp.js`** file and pass its contents **verbatim** as `use_figma` → `code` (plus `fileKey`, `description`, `skillNames: "figma-use"`). The shipping MCP tool accepts **only** inline `code` (~50k cap) — there is no `codeWorkspacePath` or file indirection. Do **not** dump the bundle via shell `cat` / `type` and copy from truncated terminal output. Bundles resolve rows + IDs in-plugin; **you never assemble `ctx` in the tool call**, and **never write a staging file** to JSON-escape the payload. Transport detail: [`conventions/16-mcp-use-figma-workflow.md`](./conventions/16-mcp-use-figma-workflow.md) § *Host vs agent transport*; bundle index: [`conventions/17-table-redraw-runbook.md`](./conventions/17-table-redraw-runbook.md). Historical stub (withdrawn file-backed RFC): [`RFC-figma-mcp-bundle-transport.md`](./RFC-figma-mcp-bundle-transport.md). Bundle map:

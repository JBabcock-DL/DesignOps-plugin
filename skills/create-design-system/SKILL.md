...
**MCP canvas runs (happy path — Steps 15a–15c + Step 17 Token Overview):** **Delegate every canvas bundle to the [`canvas-bundle-runner`](../canvas-bundle-runner/SKILL.md) subagent — do NOT `Read` `.min.mcp.js` in this (parent) thread, do NOT call `use_figma` for canvas redraws from this thread.** The parent pattern is one `Task` per page:
...
The subagent runs in an isolated context, performs exactly **one `Read`** of the matching bundle and **one `use_figma`** call with the contents verbatim, and returns a compact `{ ok, step, pageName, tableGroups, … }` summary (~200 chars). The parent logs the summary, runs the §14 audit, and advances. Rationale and token-cost math: [`conventions/16-mcp-use-figma-workflow.md`](./conventions/16-mcp-use-figma-workflow.md) § *Canvas runner subagent*. Full contract + prohibitions: [`../canvas-bundle-runner/SKILL.md`](../canvas-bundle-runner/SKILL.md). Bundle map, step → slug, and call count: [`conventions/17-table-redraw-runbook.md`](./conventions/17-table-redraw-runbook.md). 15c is always **three sequential subagent invocations** (Layout → Text Styles → Effects). Historical stub (withdrawn file-backed RFC): [`RFC-figma-mcp-bundle-transport.md`](./RFC-figma-mcp-bundle-transport.md).
...
**Canvas (Steps 15a–17):** **agent-driven, subagent-delegated** — **never** `Read` a `.min.mcp.js` bundle or call `use_figma` for a canvas bundle from this (parent) thread. For each page, emit a `Task(subagent_type: "generalPurpose")` that loads the [`canvas-bundle-runner`](../canvas-bundle-runner/SKILL.md) skill with `step=<slug>`, `fileKey`, `description`; collect the compact JSON summary, run the §14 audit with the summary (not the full bundle) in context, advance. 15c always fires **three sequential** subagent invocations (Layout → Text Styles → Effects). Follow [`conventions/16-mcp-use-figma-workflow.md`](./conventions/16-mcp-use-figma-workflow.md) § *Canvas runner subagent*, [`phases/07-steps15a-15c.md`](./phases/07-steps15a-15c.md), [`phases/06-canvas-documentation-spec.md`](./phases/06-canvas-documentation-spec.md) § A–H, and the **phase 07** convention shard list in **Conventions load map**. The no-staging-files rule (`.mcp-*` / `*-payload.json`) still applies everywhere.

...
**After style-guide tables (15a–15c, 17) — `/create-component` in stages:** If the same session will also install shadcn components and draw them in Figma, do **not** pack table redraws and multiple component draws into a single parent turn. Complete **Phase A** (Steps 15a–c + 17) first; then run **Phase B** as [`/create-component`](../create-component/SKILL.md) **one component at a time** (see [`AGENTS.md`](../../AGENTS.md) § *Session runbook*). This is an **orchestration** split only — it does not change `canvas-templates/` or table rules. **Step 6** defaults to **six** **`Task` → [`create-component-figma-slice-runner`](../create-component-figma-slice-runner/SKILL.md)** (parent-owned DAG + `handoffJson` per [`create-component/conventions/13-component-draw-orchestrator.md`](../create-component/conventions/13-component-draw-orchestrator.md)). **Legacy:** one **`Task` → [`create-component-figma-runner`](../create-component-figma-runner/SKILL.md)** (two-phase / `sixStepDraw` / one-shot). Inline `use_figma` in the parent is the **fallback** when `Task` is unavailable ([`create-component/EXECUTOR.md`](../create-component/EXECUTOR.md) §0).

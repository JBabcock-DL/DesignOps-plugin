...
**After style-guide tables (15a–15c, 17) — `/create-component` in stages:** If the same session will also install shadcn components and draw them in Figma, do **not** pack table redraws and multiple component `use_figma` calls into a single parent turn. Complete **Phase A** (Steps 15a–c + 17) first; then run **Phase B** as [`/create-component`](../create-component/SKILL.md) **one component at a time** (see [`AGENTS.md`](../../AGENTS.md) § *Session runbook*). This is an **orchestration** split only — it does not change `canvas-templates/` or table rules. For Cursor / short-output hosts, optionally delegate each component’s `use_figma` to [`../create-component-figma-runner/SKILL.md`](../create-component-figma-runner/SKILL.md) after the parent has built `CONFIG` for that component.

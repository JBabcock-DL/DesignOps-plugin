...
### E — Auto-layout and text sizing (see §0.1 and §0.2 in SKILL / `00-gotchas.md`)

Broken style guides show **empty or 1px-tall rows / cells** or **10px-tall text rows**. Both are covered authoritatively in [`../SKILL.md`](../SKILL.md) (§0) and [`../conventions/00-gotchas.md`](../conventions/00-gotchas.md):
...
- **§ 9 — Full frame sizing table** (including header, body, inline wrappers) lives in [`../conventions/08-hierarchy-and-09-autolayout.md`](../conventions/08-hierarchy-and-09-autolayout.md) §9.
...
| **Depth (subtle)** | Elevate **Theme token cards**, **Effects** tier cards, and outer **style-guide table** frames (`doc/table/{slug}` on **↳ Primitives / Theme / Layout / Text Styles / Effects**) with **`effectStyleId` → `Effect/shadow-sm`** (or one restrained **`DROP_SHADOW`** from **`shadow/color`** + **`shadow/sm/blur`** at Effects · Light) so surfaces **float slightly** — **one** shadow recipe sitewide, low opacity. **Ordering:** see **§0.4** in [`../conventions/00-gotchas.md`](../conventions/00-gotchas.md) or [`../SKILL.md`](../SKILL.md) — publish `Effect/shadow-sm` via Step 15c § 0 **before** 15a/15b on first runs. Do not stack multiple shadows. **Exception:** **`doc/table/token-overview/platform-mapping`** and its **`header` / `body` / rows / cells** stay **without** effects — the **`token-overview/platform-mapping`** section shell already provides the single elevation (**§0.9** in [`../SKILL.md`](../SKILL.md)). |
...
| **Whitespace** | Bias toward **more** padding inside cards (**28–32px**) and **48–64px** between major bands. Dense style-guide tables still use **row `minHeight` 64** + **20px** horizontal cell padding (per [`../conventions/11-cells-12-bindings-13-build-order.md`](../conventions/11-cells-12-bindings-13-build-order.md) §11) — never cram text to the cell edge. |

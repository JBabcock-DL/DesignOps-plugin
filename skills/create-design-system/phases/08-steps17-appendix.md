...
**Happy path (MCP):** Delegate to the [`canvas-bundle-runner`](../../canvas-bundle-runner/SKILL.md) subagent with `step=17-token-overview`, `fileKey`, `description`. The subagent `Read`s [`../canvas-templates/bundles/step-17-token-overview.min.mcp.js`](../canvas-templates/bundles/step-17-token-overview.min.mcp.js) and calls `use_figma` with it verbatim — **the parent thread does not `Read` the bundle or call `use_figma` directly for this step**. Collect the returned `{ ok, step, pageName, … }` summary (or `{ ok: false, skipped: 'page missing' }`). Regenerate bundles after editing [`../canvas-templates/token-overview.js`](../canvas-templates/token-overview.js): `node skills/create-design-system/scripts/bundle-canvas-mcp.mjs`. Transport + bundle index: [`../conventions/17-table-redraw-runbook.md`](../conventions/17-table-redraw-runbook.md); full subagent contract: [`../../canvas-bundle-runner/SKILL.md`](../../canvas-bundle-runner/SKILL.md).

...
| Pattern | Structure |
|---|---|
| **Mono line** (TOKEN, HEX, WEB, ANDROID, iOS, VALUE) | One Doc/Code text node. `text.resize(colWidth - 40, 1)` → `textAutoResize = 'HEIGHT'`. Read strings from `variable.codeSyntax.WEB` / `.ANDROID` / **`.iOS`** (see [`02-codesyntax.md`](./02-codesyntax.md) — not `.IOS`). **Numeric VALUE / BLUR:** if `valuesByMode[modeId]` is a **`VARIABLE_ALIAS`**, walk `getVariableById` until you reach a numeric literal — Layout `space/*` / `radius/*` and Effects `shadow/*/blur` often alias Primitives; reading only the top variable yields `—` by mistake. |
...
| **Category sub-header row** (Text Styles only) | Single row frame width 1640 minHeight 40 fill **bound** to `color/background/variant` (Theme); inner full-width cell frame must **also** bind the same fill (or `fills = []`) — **`createFrame()` defaults to white** and will look like a wrong “title row” stripe if left literal. Doc/Caption for the label (sentence case or uppercase per template); optional letter-spacing +0.08em when uppercase. |
...
2. **Create** `doc/table/{slug}/header` → `resize(1640, 56)` → append cells in column order. **Each header cell is a different species than a body cell** — see **§ 0.5** in [`00-gotchas.md`](./00-gotchas.md). Per cell: `layoutMode = 'HORIZONTAL'`; `primaryAxisSizingMode = 'FIXED'`; `counterAxisSizingMode = 'FIXED'`; `resize(colWidth, 56)`; **then** `appendChild` the label text and run **§ 0.2** (`resize(colWidth - 40, 1)` → `textAutoResize = 'HEIGHT'`). Do **not** use `VERTICAL` + `primaryAxisSizingMode = 'AUTO'` + `resize(colWidth, 1)` here — that pattern is **only** for body cells after Hug is set (**§ 0.1**).
3. **Create** `doc/table/{slug}/body` → do **not** resize; STRETCH + AUTO handles it. Leave **`body.fills = []`** (and every **`/row/*`** and **`/cell/*`** under the body, plus **`/header/cell/*`**) — `createFrame()` defaults to **opaque white**, which visually covers the table’s bound **`color/background/default`** fill and mutes header **`color/background/variant`**. Only the **outer** `doc/table/{slug}` frame, the **`/header`** row frame, **category** `cat-*` rows (variant), **swatches / preview cards** (bound fills or `effectStyleId`), and **TEXT** should retain intentional fills.
4. For each data row: **create** the row frame → set `layoutMode: 'HORIZONTAL'`, `counterAxisSizingMode: 'AUTO'` (Hug height), `primaryAxisSizingMode: 'FIXED'` (Fixed 1640 width) **before** `resize(1640, 1)` → then set `minHeight 64`, `paddingTop/paddingBottom 16`, `counterAxisAlignItems: CENTER` → append cells. For each cell, set `primaryAxisSizingMode: 'AUTO'` (Hug height) + `counterAxisSizingMode: 'FIXED'` (Fixed colWidth) **before** `resize(colWidth, 1)`, with `paddingLeft/paddingRight 20`. **See § 0.1** in [`00-gotchas.md`](./00-gotchas.md) — skipping the Hug-before-resize sequence is what produces 1px-tall rows and cells. After `appendChild`, set **`row.fills = []`** and **`cell.fills = []`** unless the cell is a special preview (see §12).

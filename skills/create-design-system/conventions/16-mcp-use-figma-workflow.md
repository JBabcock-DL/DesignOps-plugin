...
| **2 — `/create-component` Step 6 (default)** | **`Task` → [`create-component-figma-runner`](../../create-component-figma-runner/SKILL.md)** — one `Task` per component. | Parent passes `configJson`, `createComponentRoot`, registry; subagent assembles, preflights, calls `use_figma`. |

// ═══════════════════════════════════════════════════════════════════════════
// new-project / phases / 05d-token-overview.figma.js
// ═══════════════════════════════════════════════════════════════════════════
// Token Overview documentation skeleton for `/new-project` Phase 05d.
// Draws `_PageContent` on `↳ Token Overview` with `placeholder/{section}`
// nodes that `/create-design-system` Step 17 later replaces with real token
// values.
//
// Agents running Phase 05d MUST Read this file in full (no `limit`) and
// inline it VERBATIM inside a single `use_figma` call. The markdown in
// `05d-token-overview.md` used to carry this body inline, but the block
// grew past the context window that agents reliably `Read` in one pass —
// truncation silently dropped the claudeSection footer and platform-mapping
// table, producing a partial skeleton that Step 17 then tried to populate.
//
// Preconditions (from 05d-token-overview.md):
//   - `↳ Token Overview` page exists and has an `_Header` at y=0 height=320
//     cornerRadius=0 width=1800 (run Phase 05b first if missing).
//   - `fileKey` is known (Step 4 handoff or caller prompt).
//
// Idempotency: this script APPENDS a new `_PageContent` frame. If you are
// re-running on a non-empty file, run the small wipe snippet at the top of
// `05d-token-overview.md` (lines 29–38) in a separate `use_figma` call
...
if (typeof figma === 'undefined') {
  throw new Error(
    '[05d-token-overview.figma.js] `figma` global is not defined. This file must be ' +
...
if (typeof bindPrimColor !== 'function' || typeof applyDocStyle !== 'function') {
  throw new Error(
    '[05d-token-overview.figma.js] _shared-token-helpers.figma.js was not inlined. ' +
    'Read skills/new-project/phases/_shared-token-helpers.figma.js and paste its contents ' +
    'between the ↓↓↓ / ↑↑↑ markers above.'
  );
}

// 05d-specific uppercase caption variant — letterSpacing 4 differs from 05c's
// DOC_CAPTION_UC (8), so it stays in this phase file rather than the shared
...
async function buildPlatformTable(parent, slug, cols, rows) {
  ...
  // resize() MUST come before sizing-mode assignments — resize() after
  // sizing-mode set resets AUTO back to FIXED. See figma-use gotchas.

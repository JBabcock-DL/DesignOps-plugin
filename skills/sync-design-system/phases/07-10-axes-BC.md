...
**Transport:** Step 6 follows [`create-component/EXECUTOR.md`](../../create-component/EXECUTOR.md) — **default:** **six** `Task`s → **`create-component-figma-slice-runner`** (parent `handoffJson` per [§13](../../create-component/conventions/13-component-draw-orchestrator.md)); **legacy:** one `Task` → **`create-component-figma-runner`**. Parent passes **`configBlock`** + **`layout`** (not JSON-only CONFIG); inline `use_figma` is the **fallback** (including failed or interrupted `Task`s — reuse the same `configBlock`).

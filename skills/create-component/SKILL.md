...
>
> **Critical rule (shipped engine):** For **one** component, the committed draw pipeline always builds the doc frame **in section order** (header → Properties table → live ComponentSet → matrix → usage) per [`conventions/04-doc-pipeline-contract.md`](./conventions/04-doc-pipeline-contract.md). **Target MCP transport:** the **runner** issues **several** `use_figma` calls with **small, step-specific** bundles (e.g. properties table shell + headers, doc header, component tile, Variants × States, Do/Don’t) and **handoff ids** between calls — see [`conventions/09-mcp-multi-step-doc-pipeline.md`](./conventions/09-mcp-multi-step-doc-pipeline.md) and runner [**§1c**](../create-component-figma-runner/SKILL.md). **Interim:** the runner may use **two** calls with injected phase globals (**§1b** — variant plane vs doc tail) or **`twoPhaseDraw: false`** (one legacy full script); avoid treating “same full engine uploaded twice” as the long-term design.
>
> **Session sequencing (do this in separate steps — not one parent blob):** Run **style-guide** canvas bundles (**`Task` → `canvas-bundle-runner`**, one slug per Task) **before** any `/create-component` draw; use **one `Task` → `create-component-figma-runner` per component**; do not interleave unrelated large `use_figma` payloads in **one** parent turn ([`EXECUTOR.md`](./EXECUTOR.md), [`AGENTS.md`](../../AGENTS.md)). That keeps **tables vs components** separated at **orchestration** time; **inside** Step 6, prefer **many small** Figma calls once step bundles ship (still one runner `Task`).
>

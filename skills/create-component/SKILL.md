...
**🚨 Script-assembly order for Step 6 (`use_figma`):** The `code` payload MUST be assembled in this exact order or the run throws at §6.2a:

1. **§0 CONFIG object** — the per-component block below (the only per-component edit surface).
2. **`Read` and inline [`templates/draw-engine.figma.js`](./templates/draw-engine.figma.js) verbatim** (~1530 lines) — scaffold (§§1–5), chip `buildVariant`, §6 draw flow (`makeFrame`, `buildPropertiesTable`, `buildComponentSetSection`, `buildMatrix`, `buildUsageNotes`), §6.2a archetype dispatch, §6.9a self-check. Paste immediately after the §0 CONFIG.
3. **`Read` and inline [`templates/archetype-builders.figma.js`](./templates/archetype-builders.figma.js) verbatim** (~1160 lines) — the six non-chip archetype builders + shared helpers. Paste at the `↓↓↓ INLINE archetype-builders.figma.js HERE ↓↓↓` marker inside draw-engine.figma.js (between §5.7 and §6.0). MAY be skipped only when `CONFIG.layout === 'chip'`.
4. No further wrapping — §6.9a self-check is at the tail of draw-engine.figma.js and `return`s the payload.

Two runtime `typeof` asserts at the top of §6.2a (one for archetype builders, one for draw-engine functions) throw with actionable messages if either template was missed. If you only see `CONFIG` in this SKILL (because your `Read` truncated the file), assume Steps 2 and 3 above are missing. Re-`Read` both `templates/*.figma.js` files explicitly by path — do not trust a truncated SKILL.md.
...
> **AGENT ACTION REQUIRED.** The per-component editable region ends at §0 CONFIG above. Everything below §0 (scaffold, chip `buildVariant`, `§6` doc-frame pipeline, `§6.2a` dispatch, `§6.9a` self-check, and the six non-chip archetype builders) is **IDENTICAL for every component** and lives in two committed template files. You MUST `Read` both and inline them verbatim into your `use_figma` `code` payload in the order below.

**Script-assembly order for the `use_figma` call:**

1. **§0 CONFIG object** — the block above (edit per component).
2. **[`templates/draw-engine.figma.js`](./templates/draw-engine.figma.js)** (~1530 lines) — scaffold (§§1–5), chip `buildVariant`, `§6` draw flow (`makeFrame`, `buildPropertiesTable`, `buildComponentSetSection`, `buildMatrix`, `buildUsageNotes`), `§6.2a` archetype dispatch, `§6.9a` self-check. `Read` in full (no `limit`) and paste verbatim immediately after the §0 CONFIG.
3. **[`templates/archetype-builders.figma.js`](./templates/archetype-builders.figma.js)** (~1160 lines) — the six non-chip archetype builders (`buildSurfaceStackVariant`, `buildFieldVariant`, `buildRowItemVariant`, `buildTinyVariant`, `buildControlVariant`, `buildContainerVariant`, `buildComposedVariant`) plus shared helpers (`makeDashedSlot`, `makeSampleText`, `makeIconSlotShared`, `wireIconSwapProp`). `Read` in full and paste verbatim at the `↓↓↓ INLINE archetype-builders.figma.js HERE ↓↓↓` marker inside draw-engine.figma.js (between §5.7 and §6.0). You MAY skip this file only when `CONFIG.layout === 'chip'`.

**Do NOT** paraphrase, **do NOT** inline only the helpers you think you need, and **do NOT** re-declare anything already in scope from §0 CONFIG. The `§6.2a` dispatch resolves every builder by name and will throw `ReferenceError` if any is missing. Two runtime `typeof` asserts at the top of `§6.2a` (one for archetype builders, one for draw-engine functions) surface a descriptive error with the exact file path and assembly step if anything was omitted.

**Why this is a template, not inline JS:** `SKILL.md` grew past the context window that agents reliably `Read` in one call. When §6 lived inline, the `buildPropertiesTable` / `buildMatrix` / `buildUsageNotes` block would silently drop out mid-file, which caused components like Card to render with forked property tables and no Variants × States matrix. Keeping the draw engine in a dedicated file makes the `Read` step unambiguous and the failure mode loud.

For the archetype → component routing table, see [`CONVENTIONS.md §3.1.1`](./CONVENTIONS.md#311--archetype-routing-table). For the doc-pipeline contract (inputs, outputs, forbidden forks), see [`CONVENTIONS.md §3.1.3`](./CONVENTIONS.md#313--archetype-builders-must-not-touch-the-doc-pipeline).

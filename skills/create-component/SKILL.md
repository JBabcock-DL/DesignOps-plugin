...
2. **`Read` and inline [`templates/draw-engine.min.figma.js`](./templates/draw-engine.min.figma.js) verbatim** — the **minified** build of `draw-engine.figma.js` (generated by `scripts/build-min-templates.mjs`, committed to git). Contains scaffold (§§1–5), chip `buildVariant`, §6 draw flow (`makeFrame`, `buildPropertiesTable`, `buildComponentSetSection`, `buildMatrix`, `buildUsageNotes`), §6.2a archetype dispatch, §6.9a self-check. Paste immediately after the §0 CONFIG. Prefer the `.min.figma.js` variant — it is ~58% smaller and is the canonical runtime artifact. Only `Read` the non-`.min` source file when debugging.
3. **`Read` and inline [`templates/archetype-builders.min.figma.js`](./templates/archetype-builders.min.figma.js) verbatim** — the **minified** build of `archetype-builders.figma.js` (~42% smaller). Six non-chip archetype builders + shared helpers. Paste at the `↓↓↓ INLINE archetype-builders.figma.js HERE ↓↓↓` marker inside draw-engine (between §5.7 and §6.0). MAY be skipped only when `CONFIG.layout === 'chip'`.
4. No further wrapping — §6.9a self-check is at the tail of draw-engine and `return`s the payload.

Two runtime `typeof` asserts at the top of `draw-engine.figma.js §6.2a` (one for archetype builders, one for draw-engine functions) throw with actionable messages if either template was missed. The asserts name every required function (`buildSurfaceStackVariant`, `buildPropertiesTable`, etc.) — minification preserves identifier names, so the `.min` variants satisfy the asserts identically to the source files. If you only see `CONFIG` in this SKILL (because your `Read` truncated the file), assume Steps 2 and 3 above are missing. Re-`Read` both `templates/*.min.figma.js` files explicitly by path — do not trust a truncated SKILL.md.

> **Regenerating `.min` files.** Run `npm run build:min` after editing either source template. `scripts/verify-cache.sh` refuses to pass if any source is newer than its `.min` sibling. CI gate: `npm run verify` (runs `build:docs:check`, `build:min:check`, `verify-cache`).

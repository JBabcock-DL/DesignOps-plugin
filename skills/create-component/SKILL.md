...
**Step 6 — orchestrated slice draw:** The **default** path is the **parent-owned DAG** in [`conventions/13-component-draw-orchestrator.md`](./conventions/13-component-draw-orchestrator.md): **12** sequential `use_figma` invocations, each assembled per [`../create-component-figma-slice-runner/SKILL.md`](../create-component-figma-slice-runner/SKILL.md). **Engine assembly** uses `generate-ops` (`assemble-slice` default): scaffold sub-slugs emit **tuple ops + op-interpreter**; delegated slugs still inline `*.min.figma.js` today — **tuple-first** migration hook is [`scripts/op-generators/tuple-delegate-pipeline.mjs`](../../scripts/op-generators/tuple-delegate-pipeline.mjs); baseline / CI guard is **`npm run qa:tuple-parity`** ([`24-tuple-expand-delegated-roadmap.md`](./conventions/24-tuple-expand-delegated-roadmap.md)). Follow manifest disk staging per [`23-designops-step6-engine.md`](./conventions/23-designops-step6-engine.md).

The parent **may** `Read` minified engines in the main thread. **Do not** default to **`Task` → slice runner** for slices the subagent cannot emit in `call_mcp` (common). **`EXECUTOR.md`** §0 *Step 6 — transport* is authoritative.

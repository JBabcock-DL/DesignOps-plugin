...
1. **Orchestration (session / chat)** — **Do** break work up **sequentially**: finish each **style-guide** `canvas-bundle-runner` `Task` on its own; for **`/create-component`**, run **six** `Task`s → `create-component-figma-slice-runner` **or** **one** legacy `Task` → `create-component-figma-runner` **per component**; avoid one parent turn that chains unrelated large Figma calls. Tables, bundles, and component draws are **different jobs** — schedule them as **separate** steps. The parent should only pass a small **`configBlock`** + **`handoffJson`** (slice path) or runner **§0** fields, not re-assemble the minified engine in chat.
...
**Anti-pattern (still true):** Ad-hoc **minification**, **trimming** built bundles, or **stub** `code` (`PLACEHOLDER`) to “fit” MCP — that breaks `check-payload` / Figma and is forbidden per [`../../create-component-figma-runner/SKILL.md`](../../create-component-figma-runner/SKILL.md) §2.

**Allowed exception — parent preassembles, subagent only executes:** The **parent** may run the same assembly math as the runner (CONFIG + preamble + `*.stepN.min.figma.js`) via a **shell/Node** script, write each **fully assembled** `code` string to **OS temp** (or a workspace path the subagent can `Read`), run **`check-payload`** and optionally **`check-use-figma-mcp-args`**, then hand the subagent **only path list + `fileKey`** (see [`../../create-component-figma-runner/SKILL.md`](../../create-component-figma-runner/SKILL.md) **§0.1**). That keeps the **Task prompt** small and guarantees **verified** bytes — it does *not* bypass the runner’s logic, only who performs the `Read`/`concat` when the subagent’s own assembly + MCP send hits host limits.

If `Task` aborts, **retry** with `sixStepDraw: true` first, then **parent-preassembled** paths, then the documented **inline** fallback per [`../EXECUTOR.md`](../EXECUTOR.md). Follow the runner’s **§1b / §1d** chunking — do not splice or trim minified engines ad hoc outside the build pipeline.
...
3. **Never** use **`PLACEHOLDER`** in `code` to probe tool wiring — Figma throws `ReferenceError` and the runner explicitly forbids it ([`../../create-component-figma-runner/SKILL.md`](../../create-component-figma-runner/SKILL.md) §2). Use a **tiny** real script (`return { ok: true, fileKey: figma.fileKey };`) for connectivity checks.
...
**Recovery order (same component):** **six-`Task` slice chain** (default) already minimizes per-call size → legacy **`sixStepDraw: true`** on `create-component-figma-runner` → parent **preassembled** `preassembledCodePaths` per runner **§0.1** → **parent inline** `use_figma` (same `configBlock`) per [`../EXECUTOR.md`](../EXECUTOR.md) → model hop (longer context for tool args) if still failing.
...
3. **`Task` → [`create-component-figma-slice-runner`](../../create-component-figma-slice-runner/SKILL.md)** (×6, parent [`§13`](../13-component-draw-orchestrator.md)) — **default** Step 6 when subagents exist; parent never `Read`s min slices in the **parent** thread. On **subagent** transport failure, try **legacy** [`create-component-figma-runner`](../../create-component-figma-runner/SKILL.md) with **`sixStepDraw: true`** or parent-built files + **`preassembledCodePaths`** (runner **§0.1**) before parent inline / model hop.
4. **Escalation** — model hop for the Figma call only, or parent inline per [`../EXECUTOR.md`](../EXECUTOR.md) when `Task` is unavailable.

**Legacy** `create-component-figma-runner` uses phased **§1b** by default; the **slice** path uses **six** smaller payloads. For **timeout / execution-size** issues, see [`../../create-design-system/conventions/16-mcp-use-figma-workflow.md`](../../create-design-system/conventions/16-mcp-use-figma-workflow.md) (50k cap on each `code` string).
...
## Phase 2 — Default workflow: `Task` + slice runner (or legacy single-`Task` runner)
...
3. **Legacy — one `Task` per component** loading [`create-component-figma-runner`](../../create-component-figma-runner/SKILL.md): pass `fileKey`, `layout`, `configBlock`, `createComponentRoot`, and `registry` per runner **§0**; omit `twoPhaseDraw` for default **two** phased calls, or set `sixStepDraw: true` / `twoPhaseDraw: false` as needed.

4. Subagent: `Read` preamble + the **one** min row for that slice, run [`scripts/check-payload.mjs`](../../../scripts/check-payload.mjs), then **`use_figma`**. Parent runs [`SKILL.md` §9](../SKILL.md) on the **last** slice’s return (or legacy final phase) + registry.
...
- **One component per wave** — Prefer install → 4.7 → **six** slice `Task`s (default) or **one** legacy runner `Task` (or one inline assembly) → §9 + registry before starting the next component in a **new** turn if output limits bite ([`../EXECUTOR.md`](../EXECUTOR.md) *Session / output limits*).

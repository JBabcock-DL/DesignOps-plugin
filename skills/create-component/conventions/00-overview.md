...
| **state-layer frame** | One of three absolutely-positioned `FrameNode` children inside each variant component: `state-layer/hover`, `state-layer/pressed`, `state-layer/focus`. Each has `layoutPositioning = 'ABSOLUTE'`, `constraints SCALE/SCALE`, `opacity = 0`, and a fill bound to the corresponding `color/state/{stateRole}/{state}` Theme variable. `applyStateOverride` sets `opacity = 1` on the matching layer for the active state; all others stay at 0. Disabled state skips state layers and sets `instance.opacity = 0.38` instead (M3 whole-component desaturation). `stateRole` is derived from `CONFIG.style[variant].stateRole` or parsed from the fill path. |
...
| **state override** | A mutation applied to a matrix cell's instance at draw time to simulate `:hover` / `:active` / `:disabled`. For button-like components the authoritative mechanism is **state-layer visibility** — `applyStateOverride` hides all `state-layer/*` child frames then sets `opacity = 1` on `state-layer/{stateKey}`. Disabled bypasses layers and sets `instance.opacity = 0.38`. `setProperties({...})` is the exception for components where state IS a Figma variant (checkbox, switch). See [`04-doc-pipeline-contract.md`](./04-doc-pipeline-contract.md) §13.1. |

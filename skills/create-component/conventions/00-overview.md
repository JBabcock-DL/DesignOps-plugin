...
>
> **Authoritative source:** [`skills/create-component/SKILL.md`](../SKILL.md) (**§9** gate) and [`skills/create-component/EXECUTOR.md`](../EXECUTOR.md) (Steps **1–8** + five-call draw). When any sub-file disagrees with the skill router, **EXECUTOR** wins assembly/transport; **conventions/** win geometry.
>
...
| **Step 6 `use_figma` transport** (parent `Read` → `call_mcp`, payload caps, ephemeral staging) | [`skills/create-component/EXECUTOR.md`](../EXECUTOR.md), [`skills/create-design-system/conventions/16-mcp-use-figma-workflow.md`](../../create-design-system/conventions/16-mcp-use-figma-workflow.md) |
| **Five-call component bundles** (`scaffold`, `properties`, `component-*`, `matrix`, `usage`) | [`../canvas-templates/bundles/`](../canvas-templates/bundles/), `npm run bundle-component` |
...
| **state-layer frame** | One of three absolutely-positioned `FrameNode` children inside each variant component: `state-layer/hover`, `state-layer/pressed`, `state-layer/focus`. Each has `layoutPositioning = 'ABSOLUTE'`, `constraints STRETCH/STRETCH`, `opacity = 0`, and a fill bound to the `color/state/{stateRole}/{state}` Theme variable. `stateRole` resolves to `on-{role}` for filled variants (overlay = on-color) or `on-surface` for outline/ghost/transparent variants (overlay = on-surface, Light ≠ Dark). M3-strict: the state layer always carries the foreground (on-color) of the container it sits on. The legacy bare-role overlay family (`color/state/{role}/*`) is removed. `applyStateOverride` sets `opacity = 1` on the matching layer for the active state; all others stay at 0. Disabled state skips state layers, hides all `state-layer/*` and `focus-ring` child frames, and sets `instance.opacity = 0.38` instead (M3 whole-component desaturation). |
| **focus-ring frame** | A single absolutely-positioned `FrameNode` child named `focus-ring`, sibling to the three `state-layer/*` frames. `strokeAlign: 'OUTSIDE'` with `strokeWeight: 2` and stroke bound to `color/component/ring` (override via `style[v].focusRingVar`). Corner radii match the parent component. `opacity = 0` by default; `applyStateOverride` sets it to 1 when `stateKey === 'focus'`. Parent component has `clipsContent = false` so the outside stroke renders past the component edge. |
...
| **state override** | A mutation applied to a matrix cell's instance at draw time to simulate `:hover` / `:active` / `:focus` / `:disabled`. For button-like components the authoritative mechanism is **state-layer + focus-ring visibility** — `applyStateOverride` hides all `state-layer/*` and `focus-ring` child frames, then sets `opacity = 1` on `state-layer/{stateKey}` (and additionally `focus-ring` when `stateKey === 'focus'`). Disabled bypasses layers, hides both frame families, and sets `instance.opacity = 0.38`. `setProperties({...})` is the exception for components where state IS a Figma variant (checkbox, switch). See [`04-doc-pipeline-contract.md`](./04-doc-pipeline-contract.md) §13.1. |
...
| `use_figma` transport + Step 6 (five-call) | [`../EXECUTOR.md`](../EXECUTOR.md), [`../../create-design-system/conventions/16-mcp-use-figma-workflow.md`](../../create-design-system/conventions/16-mcp-use-figma-workflow.md) |

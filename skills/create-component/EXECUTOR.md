...
1. **Default — `Task` → [`create-component-figma-runner/SKILL.md`](../create-component-figma-runner/SKILL.md)** whenever the host exposes a **subagent `Task`** (Claude Code, Cursor, etc.). The parent completes Steps **1–5** and **4.7**, then serializes the finalized **`CONFIG`** to **`configJson`** (JSON string) and invokes **one `Task` per component** with **`fileKey`**, **`createComponentRoot`** (path to the folder containing `templates/preamble.figma.js` — typically `…/skills/create-component/`), and **registry** inputs per the runner’s **§0**. The parent **must not** `Read` the `*.min.figma.js` engine into its own thread for that draw. **Step 5.5** (`check-payload` / full wrapper checks) runs **inside the subagent** during assembly. The parent runs **`SKILL.md` §9** on the compact JSON the subagent returns (plus any `raw` fields needed for assertions).
...
| 5.5 | Local syntax + JSON-transport preflight | [`scripts/check-payload.mjs`](../../scripts/check-payload.mjs) (or `npm run check-payload`) | **Runner path:** subagent runs this on the assembled `code` per [`create-component-figma-runner` §1](../create-component-figma-runner/SKILL.md). **Inline path:** parent runs on CONFIG + preamble + engine (full `code` string). Script exits 0 with `OK …`; if it exits 1, **never** submit. |
| 6 | Draw component → Figma | **`Task` → `create-component-figma-runner` (default)** or **`use_figma`** (fallback) | **Runner:** `fileKey`, `configJson`, `createComponentRoot`, registry per runner §0. **Inline:** `fileKey`, assembled `code` per §6 template below. | One successful Figma draw per component; return payload / compact JSON sufficient for **`SKILL.md` §9** assertions. |

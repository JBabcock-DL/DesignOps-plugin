...
**Tools you will use:** `AskUserQuestion`, `Shell` (for `npx shadcn@latest` + file reads), `Read` / `Glob` / `Grep`, **`Task` → [`create-component-figma-runner/SKILL.md`](../create-component-figma-runner/SKILL.md) (default Step 6 when subagents exist)**, `use_figma` (fallback inline path only), `get_screenshot` (final visual check).
...
**Step 6 — transport (runner-first):**

1. **Default — `Task` → [`create-component-figma-runner/SKILL.md`](../create-component-figma-runner/SKILL.md)** whenever the host exposes a **subagent `Task`** (Claude Code, Cursor, etc.). The parent completes Steps **1–5** and **4.7**, then copies the finalized Step 6 **`const CONFIG = { … };`** block (**`configBlock`**) **verbatim** — the same JavaScript it would paste first on the inline path, including **`applyStateOverride`** and any function-valued keys (`JSON.stringify` **drops** those; do not use JSON-only transport). Pass **`layout`** (= `CONFIG.layout`) plus **`fileKey`**, **`createComponentRoot`** (path to the folder containing `templates/preamble.figma.js` — typically `…/skills/create-component/`), and **registry** per the runner’s **§0**. The parent **must not** `Read` the `*.min.figma.js` engine into its own thread for that draw. **Step 5.5** (`check-payload` / full wrapper checks) runs **inside the subagent** during assembly. The parent runs **`SKILL.md` §9** on the compact JSON the subagent returns (plus any `raw` fields needed for assertions).

2. **Fallback — inline `use_figma` in the parent** only when **`Task` / subagent is unavailable** or the designer **explicitly** requests a single-thread draw. Follow the **inline assembly order** below; the parent runs **Step 5.5** before submitting `use_figma`.

**🚨 Inline fallback — script-assembly order for Step 6 (`use_figma`):** Use **only** when **not** delegating to `create-component-figma-runner`. The `code` payload MUST be assembled in this exact order. Three steps — skipping any of them throws a clear, actionable error at the top of the engine bundle's preamble-presence gate.
...
| **Session / output limits** | Prefer finishing **install → 4.5 → 4.7 → one `Task` (runner) or inline assemble → `check-payload` (subagent or parent) → `use_figma` → registry** for **one component** before starting the next in a **new** assistant turn. Do not assume a single parent message can draw four full components end-to-end without truncation. |
...
| **Sequential orchestration vs phased Step 6** | **Orchestration:** Treat **tables**, **each style-guide bundle**, and **each component** as **separate** Tasks / turns — never one parent message that mixes them. **Per-component draw — target:** [`create-component-figma-runner`](../create-component-figma-runner/SKILL.md) should issue **many** small `use_figma` calls (variant plane → properties table shell → doc header → component tile → matrix → usage) with **different** min bundles per call and **handoff ids** between calls — see [`conventions/09-mcp-multi-step-doc-pipeline.md`](./conventions/09-mcp-multi-step-doc-pipeline.md). **Interim / bridge:** runner may use **two** calls (**§1b**: ComponentSet, then doc tail) or **`twoPhaseDraw: false`** (one full script). **Shipped:** `create-component-engine-{layout}.step0.min.figma.js` plus shared **`create-component-engine-doc.step1`…`step5.min.figma.js`** from `npm run build:min` — **§1d** six-call mode (`sixStepDraw: true`); **step 0** and **doc** slices omit variant-build paths (~14–21 KB step0 + **~17–23 KB** doc files after esbuild + **terser** unused strip vs ~32–38 KB full engine). **Anti-pattern:** assuming six calls always minimize **total** bytes vs two-phase. Prefer **runner** so bundles never transit the parent thread ([`conventions/08-cursor-composer-mcp.md`](./conventions/08-cursor-composer-mcp.md)). |
...
| **Step 6 default — `create-component-figma-runner`** | When **`Task` exists**, **always** delegate Step 6 per [`../create-component-figma-runner/SKILL.md`](../create-component-figma-runner/SKILL.md) — parent passes `fileKey`, `layout`, `configBlock`, `createComponentRoot`, registry; subagent assembles, runs `check-payload`, calls `use_figma`. **Fallback:** inline assembly in the parent only when `Task` is unavailable. |
...
| 5.5 | Local syntax + full-code preflight | [`scripts/check-payload.mjs`](../../scripts/check-payload.mjs) (or `npm run check-payload`) | **Runner path:** subagent runs this on the assembled `code` per [`create-component-figma-runner` §1](../create-component-figma-runner/SKILL.md). **Inline path:** parent runs on CONFIG + preamble + engine (full `code` string). Script exits 0 with `OK …`; if it exits 1, **never** submit. |
| 6 | Draw component → Figma | **`Task` → `create-component-figma-runner` (default)** or **`use_figma`** (fallback) | **Runner:** `fileKey`, `layout`, `configBlock`, `createComponentRoot`, registry per runner §0. **Inline:** `fileKey`, assembled `code` per §6 template below. | One successful Figma draw per component; return payload / compact JSON sufficient for **`SKILL.md` §9** assertions. |

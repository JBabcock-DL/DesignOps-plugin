...
| **Sequential orchestration vs phased Step 6** | **Orchestration:** Treat **tables**, **each style-guide bundle**, and **each component** as **separate** Tasks / turns — never one parent message that mixes them. **Per-component draw (default):** [`create-component-figma-runner`](../create-component-figma-runner/SKILL.md) runs **two** `use_figma` calls with the **same** min bundle — phase 1 ComponentSet, phase 2 doc pipeline (**§1b** globals). **`twoPhaseDraw: false`** — legacy **one** call (full script). Prefer **runner** so the ~40K bundle never transits the parent thread ([`conventions/08-cursor-composer-mcp.md`](./conventions/08-cursor-composer-mcp.md)). |

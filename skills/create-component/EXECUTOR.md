...
**Tools you will use:** `AskUserQuestion`, `Shell` (for `npx shadcn@latest` + file reads), `Read` / `Glob` / `Grep`, **`Task` → [`create-component-figma-slice-runner/SKILL.md`](../create-component-figma-slice-runner/SKILL.md) (Step 6 — **six** sequential `Task`s)**, `use_figma` (inline only when not delegating — see **§0** below), `get_screenshot` (final visual check).
...
1. **Six sequential `Task`s → [`create-component-figma-slice-runner/SKILL.md`](../create-component-figma-slice-runner/SKILL.md)** whenever the host exposes **`Task`**. The parent completes Steps **1–5** and **4.7**, then runs the fixed DAG in [`conventions/13-component-draw-orchestrator.md`](./conventions/13-component-draw-orchestrator.md) **§1** — `cc-variants` → `cc-doc-props` → `cc-doc-component` → `cc-doc-matrix` → `cc-doc-usage` → `cc-doc-finalize` — **one `Task` per slug**. Each prompt passes the same **`configBlock`** (verbatim `const CONFIG = { … };`), **`layout`**, **`fileKey`**, **`createComponentRoot`**, **registry** (as [slice runner §0](../create-component-figma-slice-runner/SKILL.md)), and **`handoffJson`** built from the **previous** slice return (first slice: `{}`). The parent **must not** `Read` `*.min.figma.js` engines into the main thread. **Step 5.5** / `check-payload` runs **inside** each subagent. On the **final** slice (`cc-doc-finalize`) success, the parent runs **`SKILL.md` §9** and **5.2** on that return only.

2. **Fallback — inline or preassembled `use_figma` in the parent** when **`Task` is unavailable**, interrupted, or the host cannot run six `Task`s — same **`configBlock`** and assembly rules, **no** re-derive CONFIG:
   - **2a — Inline** — follow the **inline assembly order** below; parent runs **Step 5.5** before each `use_figma` (two **§1b**-style phased calls with full per-archetype `*.min.figma.js` are the usual inline parity with the old two-phase run).
   - **2b — Preassembled (rare)** — parent (or a script) writes **fully assembled** `code` strings to **OS temp** or a workspace path, in DAG order. For each file: `Read` → `check-payload` → `use_figma`. Same bytes the slice subagent would build; use when the **`Task` JSON envelope** is too small for the subagent to pass `code` — see [`08-cursor-composer-mcp.md`](./conventions/08-cursor-composer-mcp.md).

**🚨 Inline / preassembled — script-assembly order for Step 6 (`use_figma`):** Use for **2a/2b** when **not** using the **six-`Task` slice** chain. The `code` payload MUST be assembled in this exact order. Three steps — skipping any of them throws a clear, actionable error at the top of the engine bundle's preamble-presence gate.
...
| **Session / output limits** | Prefer finishing **install → 4.5 → 4.7 → six `Task`s (slice runner) or inline / preassembled** per component; then **registry** — before the next component or in a new turn if context is tight. |
...
| **Sequential orchestration vs phased Step 6** | **Orchestration:** Treat **tables**, **each style-guide bundle**, and **each component** as **separate** Tasks / turns — never one parent message that mixes them. **Per-component draw:** **six** `Task`s → [`create-component-figma-slice-runner`](../create-component-figma-slice-runner/SKILL.md) with **handoff JSON** per [`conventions/13`](./conventions/13-component-draw-orchestrator.md) and [`09`](./conventions/09-mcp-multi-step-doc-pipeline.md). **Shipped** min slices: `create-component-engine-{layout}.step0.min.figma.js` + `create-component-engine-doc.step1`…`step5`. **Anti-pattern:** `Read`ing min engines into the parent thread. |
...
| **Step 6 subagent** | The **only** delegated Figma subagent is [`../create-component-figma-slice-runner/SKILL.md`](../create-component-figma-slice-runner/SKILL.md) (six `Task`s / DAG). **Fallback:** inline or preassembled in parent per **§0** above. |
...
| 6 | Draw component → Figma | **`Task` → `create-component-figma-slice-runner` ×6** or **inline / preassembled `use_figma`** (§0) | **Six `Task`s:** per [`conventions/13`](./conventions/13-component-draw-orchestrator.md) + slice **§0**. **Inline:** `fileKey`, assembled `code` per template table below. | **Slice path:** last slice return for §9. **Inline / preassembled:** final call’s return payload. **Per-slice agent chapters (required):** [`phases/04-slice-cc-variants.md`](./phases/04-slice-cc-variants.md) → [`09-slice-cc-doc-finalize.md`](./phases/09-slice-cc-doc-finalize.md) — parent reads the matching file **before** each `Task` for that `step`. |

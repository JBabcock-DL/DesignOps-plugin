...
**MCP payloads:** Each `use_figma` invocation must pass its Plugin API script **inline** in the tool’s `code` field (built from [`SKILL.md`](./SKILL.md) + committed `templates/*.figma.js` where referenced). Do **not** add throwaway `.mcp-*` / `*-payload.json` / scratch copies under the repo to stage that script — see [`AGENTS.md`](../../AGENTS.md).
...
| **Delegated Figma draw (optional)** | On hosts that clip large tool args in the **parent** message, emit `Task` → [`../create-component-figma-runner/SKILL.md`](../create-component-figma-runner/SKILL.md) with `fileKey`, `configJson`, `createComponentRoot`, and registry fields — the subagent assembles and calls `use_figma`; the parent does not paste the minified engine. |
...
| 6 | Draw component → Figma | `use_figma` (one call per component) | `fileKey`, `CONFIG` block per `SKILL.md` §6, optional `ICON_PACK.figmaIconLibraryKey` + `ICON_PACK.defaultIconRef` | Return payload with `{ compSetId, compSetVariants, compSetPropertyDefinitions, firstVariantChildren, iconVariantChildren, propErrorsCount, iconSlotMode: "placeholder" \| "instance-swap", iconPackResolution: "by-key" \| "by-node-id" \| "failed:*", … }` |

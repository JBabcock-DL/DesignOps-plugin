...
> **Ramping up a new agent (Claude Sonnet, Opus, future models)?** **Claude Code** loads [`CLAUDE.md`](CLAUDE.md) automatically for this repo — it points to [`memory.md`](memory.md) (full workflow index + subagent/session rules) and [`AGENTS.md`](AGENTS.md) (MCP + cache policy). **Cursor:** agents should read [`memory.md`](memory.md) early when this plugin is in the workspace (see [`.cursor/rules/cursor-designops-skill-root.mdc`](.cursor/rules/cursor-designops-skill-root.mdc)).
>
...
`/create-component`'s committed templates include **per-archetype full engines** (`create-component-engine-{layout}.min.figma.js`, ~32–35 KB) and **min slices** for the six-step ladder (`*.step0` + `create-component-engine-doc.step1`…`step5`, much smaller per call). The **default** transport is **six** `Task`s → [`create-component-figma-slice-runner`](skills/create-component-figma-slice-runner/SKILL.md) (one min slice + preamble + CONFIG per `use_figma`; see [`conventions/13-component-draw-orchestrator.md`](skills/create-component/conventions/13-component-draw-orchestrator.md)). **Legacy** [`create-component-figma-runner`](skills/create-component-figma-runner/SKILL.md): **two** full-engine calls (phased), **`sixStepDraw: true`**, or **`twoPhaseDraw: false`** (assembly order: [`skills/create-component/EXECUTOR.md`](skills/create-component/EXECUTOR.md)). **Inline** in the parent uses the full per-archetype file from the table below.

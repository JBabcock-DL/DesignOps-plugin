...
- **Phase B — Components:** Run `/create-component` **one component at a time** to completion (install + preflight + Step 6 through the **default six-slice** chain or **legacy** runner, then registry / reporting) before starting the next, across **as many assistant turns as needed** — see [`skills/create-component/EXECUTOR.md`](skills/create-component/EXECUTOR.md) *Session / output limits*.
- **Orchestration vs engine:** Staging **tables**, **bundles**, and **components** in **separate** Tasks/turns is required. **Inside** Step 6, the **default** is **six** sequential `Task`s → [`skills/create-component-figma-slice-runner/SKILL.md`](skills/create-component-figma-slice-runner/SKILL.md) (one Figma `use_figma` per `Task`, parent merges handoff JSON between them) per [`skills/create-component/conventions/13-component-draw-orchestrator.md`](skills/create-component/conventions/13-component-draw-orchestrator.md). **Legacy:** one `Task` → [`skills/create-component-figma-runner/SKILL.md`](skills/create-component-figma-runner/SKILL.md) with **two** phased calls (**§1b**), or **`sixStepDraw: true`**, or **`twoPhaseDraw: false`** — see [`skills/create-component/conventions/08-cursor-composer-mcp.md`](skills/create-component/conventions/08-cursor-composer-mcp.md) *Sequential work vs one payload*.

**`/create-component` Step 6 (default):** When **`Task`** is available, the parent **must** run the **orchestrated draw** after Steps **1–5** and **4.7**: **six** sequential `Task`s → [`create-component-figma-slice-runner` §0](skills/create-component-figma-slice-runner/SKILL.md) with `step=cc-variants` through `cc-doc-finalize` and **`handoffJson`** between calls; it **must not** paste minified `*.min.figma.js` into the parent thread. **Legacy:** a **single** `Task` → [`skills/create-component-figma-runner/SKILL.md`](skills/create-component-figma-runner/SKILL.md) (two-phase, six internal steps, or one call). Subagents run `check-payload` before each `use_figma`. **`Fallback`** — inline / interrupted handling per [`skills/create-component/EXECUTOR.md`](skills/create-component/EXECUTOR.md) §0. Style-guide canvas bundles **still** use **`canvas-bundle-runner` only** — not component runners for Step 15/17.

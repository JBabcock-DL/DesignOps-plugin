...
- **Phase B — Components:** Run `/create-component` **one component at a time** to completion (install + preflight + Step 6 **six-slice** chain or `EXECUTOR` inline/preassembled fallback, then registry / reporting) before starting the next, across **as many assistant turns as needed** — follow [`skills/create-component/phases/00-index.md`](skills/create-component/phases/00-index.md) for time order; see [`skills/create-component/EXECUTOR.md`](skills/create-component/EXECUTOR.md) *Session / output limits*.
- **Orchestration vs engine:** Staging **tables**, **bundles**, and **components** in **separate** Tasks/turns is required. **Step 6** = **six** sequential `Task`s → [`skills/create-component-figma-slice-runner/SKILL.md`](skills/create-component-figma-slice-runner/SKILL.md) (one Figma `use_figma` per `Task`, parent merges handoff JSON) per [`skills/create-component/conventions/13-component-draw-orchestrator.md`](skills/create-component/conventions/13-component-draw-orchestrator.md) — the **only** Figma subagent for components. **Fallback:** parent inline or preassembled per [`skills/create-component/EXECUTOR.md`](skills/create-component/EXECUTOR.md) **§0**. See [`skills/create-component/conventions/08-cursor-composer-mcp.md`](skills/create-component/conventions/08-cursor-composer-mcp.md) *Sequential work vs one payload*.

**`/create-component` Step 6:** When **`Task`** is available, the parent **must** run the **orchestrated draw** after Steps **1–5** and **4.7**: **six** sequential `Task`s → [`create-component-figma-slice-runner` §0](skills/create-component-figma-slice-runner/SKILL.md) with `step=cc-variants` through `cc-doc-finalize` and **`handoffJson`** between calls; it **must not** paste minified `*.min.figma.js` into the parent thread. Subagents run `check-payload` before each `use_figma`. **`Fallback`** — inline / preassembled / interrupted handling per [`skills/create-component/EXECUTOR.md`](skills/create-component/EXECUTOR.md) §0. Style-guide canvas bundles **still** use **`canvas-bundle-runner` only** — not component slice `Task`s for Step 15/17.

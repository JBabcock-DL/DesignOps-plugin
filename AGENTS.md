...
**Agents run plugin CLI:** Documented **`node scripts/…`** and **`npm run …`** steps (e.g. **`bundle-component`**, **`verify`**, **`merge-handoff`** where applicable) are **executed by the agent** from the DesignOps-plugin root unless blocked (sandbox/network, interactive auth, or no working `call_mcp` / Figma MCP in the environment). Cursor: [`.cursor/rules/agent-run-designops-commands.mdc`](.cursor/rules/agent-run-designops-commands.mdc).

## Step 6 Figma invoke — default is **Cursor / Claude Code Figma MCP** (`call_mcp`), not standalone Node MCP

**`/create-component` Step 6:** Parent assembles **`ctx` +** the committed bundle from [`skills/create-component/canvas-templates/bundles/*.min.mcp.js`](skills/create-component/canvas-templates/bundles/) and runs **five** sequential **`call_mcp` → `use_figma`** invocations per [`skills/create-component/EXECUTOR.md`](skills/create-component/EXECUTOR.md) **§0**. Regenerate bundles after editing canvas sources: **`npm run bundle-component`**. Resolver Figma MCP `server` id from workspace **`mcps/*/SERVER_METADATA.json`** — do **not** hardcode a server name across hosts.
...
`Task` **writers** may assemble `code` to disk (**`check-payload`**, **`Write`**); **parent** owns **`call_mcp` `use_figma`**. Detail: **[`EXECUTOR.md`](skills/create-component/EXECUTOR.md)** §0 and **[`16-mcp-use-figma-workflow.md`](skills/create-design-system/conventions/16-mcp-use-figma-workflow.md)** (non-canvas section).
...
- **Parent still performs `call_mcp`:** Subagent **`Write` → disk → parent `Read` → one `call_mcp`** is the approved pattern ([`16` — non-canvas / optional writer flow](skills/create-design-system/conventions/16-mcp-use-figma-workflow.md)). The **consumer design repo**, a **session `draw/` or `tmp/` folder**, or **OS temp** are preferred staging paths; keep them **gitignored** and **delete** when done unless debugging.
...
- **Components (`/create-component` Step 6):** **Default = parent** **five-call** loop per [`skills/create-component/EXECUTOR.md`](skills/create-component/EXECUTOR.md) **§0** — committed **`.min.mcp.js`** under [`canvas-templates/bundles/`](skills/create-component/canvas-templates/bundles/). For **non-canvas** one-off scripts, follow [`16`](skills/create-design-system/conventions/16-mcp-use-figma-workflow.md) — split into multiple `use_figma` calls if needed; do not rely on a subagent to emit oversized `call_mcp` payloads.
...
- **Phase B — Components:** Run `/create-component` **one component at a time** to completion (install + preflight + Step 6 **five** `use_figma` invocations in **parent**, then registry / reporting) before starting the next, across **as many assistant turns as needed** — see [`skills/create-component/EXECUTOR.md`](skills/create-component/EXECUTOR.md).
- **Orchestration vs engine:** Staging **tables**, **bundles**, and **components** in **separate** turns is required. **Step 6** = **five** sequential bundles ([`EXECUTOR` §0](skills/create-component/EXECUTOR.md)) — **default:** parent **`Read`** committed `.min.mcp.js` and **`call_mcp`**. **Do not** default to `Task` subagents for payloads they cannot emit. See [`16`](skills/create-design-system/conventions/16-mcp-use-figma-workflow.md).

**`/create-component` Step 6:** After Steps **1–5** and **4.7**, run **five** `use_figma` invocations in order (`scaffold` → `properties` → `component-*` → `matrix` → `usage`) per [`EXECUTOR` §0](skills/create-component/EXECUTOR.md). `check-payload` before each `use_figma`. Style-guide canvas bundles use **`canvas-bundle-runner`** (or [16](skills/create-design-system/conventions/16-mcp-use-figma-workflow.md) fallback) — not component-specific runners for Step 15/17.
...
**Supported path:** Put the assembled **`code`** (from committed **`Read(...)`** of **`.min.mcp.js`**) in the MCP tool argument’s **`code`** field. **`Read`** preserves full-length bytes; shell `cat` / `type` can truncate — do **not** use those as proof of correctness.

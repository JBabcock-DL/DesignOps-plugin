...
for (const layout of LAYOUTS) {
  ...
  const doc = [1, 2, 3, 4, 5].map(n => b(resolve(T, `create-component-engine-doc.step${n}.min.figma.js`)));
  const sixEng = s0 + doc.reduce((a, x) => a + x, 0);
  const sixNaive = sixEng + 6 * pBytes;
  ...
  console.log(`  6 step engines sum: ${sixEng}`);
  console.log(`  6 step +6 premble:  ${sixNaive}  (naive σ if full preamble each time)`);
  console.log('');
}
...
console.log(
  `---\n` +
    `Target σ ~${TARGET}: naive six-step above is NOT a byte partition of one monolith — ` +
    `build recompiles overlapping draw-engine; see skills/create-component/conventions/12-sigma-budget-mcp.md ` +
    `(partitioned entries + thin preamble 2–6 can approach monolith total).\n`,
);

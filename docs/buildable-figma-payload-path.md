# Buildable path: large `use_figma` when the IDE cannot serialize full tool args

**Product default:** **Cursor** or **Claude Code** Figma MCP — parent **`Read`** staging JSON (from **`assemble-slice --emit-mcp-args`**) then **`call_mcp` → `use_figma`**. That uses the **editor-managed** connector and session.

This document covers what to do when **that** path fails (truncation, headless CI) or you need a **short shell line** without composing multikilobyte JSON in chat: **Node** reads the same JSON from disk and calls **Figma Desktop MCP URL** via **`@modelcontextprotocol/sdk`**. That is **not** a replacement for the Cursor/Claude integration when the IDE is the MCP host — see **[`23-designops-step6-engine.md`](../skills/create-component/conventions/23-designops-step6-engine.md)** for **`finalizeHint.fallbackShellPipe`**.
...
|----------|------------------------------|-----------------------------------|
| Cursor / Claude **`Read` + `call_mcp` `use_figma`** | No `codeFile`; uses editor Figma MCP | Parent / IDE (staging JSON on disk) |
| Desktop MCP URL + **`figma:mcp-invoke`** | No (same bytes; Node client, not IDE) | Node (this script) |

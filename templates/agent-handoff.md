...
**Canvas vs variables:** Figma **variables** (collections, modes, `codeSyntax`) are the source of truth for code and dev mode. **Canvas frames** (doc headers on each page, style guide swatches, `[MCP] Token Manifest`, Token Overview copy, Thumbnail `Cover`) are generated by `use_figma` per the skills (`/create-design-system` Steps 15a–18; `/sync-design-system` Steps 9b–9e after a push to Figma). If a file was created before a skill update, re-run the relevant skill steps or the whole skill so canvas and variables stay aligned.

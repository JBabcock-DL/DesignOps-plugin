...
| Create Component | `/create-component` | Installs shadcn/ui components, wires `tokens.css` into `globals.css`, draws each component into a **3-section documentation frame (matrix-default)** on its Foundations page — header + properties-and-types table (1640 wide) + variant × state specimen matrix (rows = variant, columns = state grouped DEFAULT / DISABLED, size groups stacked) + Do/Don't usage notes — with the `ComponentSet` parked at y=-2000 for Code Connect resolution. See `skills/create-component/CONVENTIONS.md` for the full layout spec, per-category state axes, and a button reference implementation |

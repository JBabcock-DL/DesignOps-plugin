...
| Sync Design System | `/sync-design-system` | Diffs a local token file against live Figma variable state and pushes changes in either direction; after pushes to Figma, runs **9b / 9d / 9e** (same premium style guide tables + Token Overview refresh + Thumbnail `Cover` as **create-design-system**) so canvas docs stay aligned with variables. Optional **`↳ changelog`** page (Step **9f**, designer opt-in). After **Figma→code** file writes, optional **git publish** (skip / PR / push current branch) per [`skills/sync-design-system/reference/git-publish-after-figma-code.md`](skills/sync-design-system/reference/git-publish-after-figma-code.md). |
| Create Component | `/create-component` | Installs shadcn/ui components, wires `tokens.css` into `globals.css`, draws each component into the **standard 5-section documentation frame (matrix-default)** on its Foundations page — header + properties-and-types table (1640 wide) + **live Component Set section (editable, horizontal-wrap grid)** + variant × state specimen matrix (rows = variant, columns = state grouped DEFAULT / DISABLED, size groups stacked) + Do/Don't usage notes. The `ComponentSet` lives inline in the doc frame (not parked off-canvas) so designers can edit variants directly; matrix cells are instances that update from it automatically. Inner variant labels bind to published `Label/*` text styles via `CONFIG.labelStyle`. The same layout applies to every component shape (button-like, input-like, checkable, overlay, layout) — a per-component `CONFIG` object drives the draw engine; see `skills/create-component/conventions/01-config-schema.md` for the `CONFIG` schema and per-category state axes |
...
| /sync-design-system | `/sync-design-system` | none (interactive) | Diffs and syncs Figma variables with local tokens; redraws style guide pages + Token Overview + cover after pushes to Figma; optional **`↳ changelog`** (Step **9f**) |
| /create-component | `/create-component [components...]` | components: shadcn component names | Installs components, wires tokens.css, draws each into a matrix-default doc frame (header + properties table + variant × state matrix + usage notes) per `skills/create-component/conventions/` |
